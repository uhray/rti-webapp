...
<section class="UserTable">
    ...
    <p class='UserTable-deleteMulti' on:click={() => modalOpen = true}>Delete {type}s</p>
    {/if}
  {#if modalOpen}
  <DeleteModal on:closeModal={closeModal} type={modalType} on:confirm={() => deleteUsers(selectedUsers)}/>
  {/if}  
                  ...
                  <td class="UserTable-rowIcons"
                    ...
                    <div>
                      <Icon
                        type="delete"
                        hover
                        on:delete={() => handleDelete(item._id, 'admin', 'single')}
                      />
                    </div>
                    <div>
                      <input
            ...
            {:else}
              {#each data as item}
                <tr class="UserTable-row user">
                  ...
                  <td class="UserTable-rowIcons">
                    {#if item._id !== $userStore.user._id}
                    <div><Icon type="edit" hover /></div>
                    <div>
                        
                      <Icon
                        type="delete"
                        hover
                        on:delete={() => handleDelete(item._id, 'user', 'single')}
                      />
